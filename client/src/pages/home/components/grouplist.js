...
import { setAllGroups, setSelectedGroup } from './../../../redux/usersSlice';
import moment from "moment";
import { useEffect } from "react";
import store from "../../../redux/store";

function Grouplist({socket}){
    const { allgroups, user: currentUser, selectedGroup } = useSelector(state => state.userReducer);
    const dispatch = useDispatch();

   


     const openGroup = (selectedUserId) => {
            ...
            const openChat = (selectedGroupId) => {
            const chat = allChats.find(chat => {
                // Check if the chat is a group chat
                const memberIds = chat.members.map(m => m._id);
                
                // For group chats, check if the current user is in the chat
                if (memberIds.includes(currentUser._id)) {
                    // If it's a group chat, we don't need to check for a specific user ID
                    return memberIds.length > 2; // Assuming group chat has more than 2 members
                } else {
                    // For individual chats, ensure both users are included
                    return memberIds.includes(selectedUserId) && memberIds.length === 2;
                }
            });
